		...
		try {
			...
			switch (name) {
				case 'search_apis_by_category':
					return await this.searchService.searchAPIsByCategory(
						String(args.category || ''),
						Number(args.limit || 10)
					)

				case 'search_apis_by_keyword':
					return await this.searchService.searchAPIsByKeyword(
						String(args.keyword || ''),
						Number(args.limit || 10)
					)

				case 'filter_apis_by_auth':
					return await this.searchService.filterAPIsByAuth(
						String(args.authType || ''),
						Number(args.limit || 10)
					)

				case 'filter_apis_by_https':
					return await this.searchService.filterAPIsByHTTPS(
						args.httpsOnly !== false,
						Number(args.limit || 10)
					)

				case 'filter_apis_by_cors':
					return await this.searchService.filterAPIsByCORS(
						String(args.corsSupport || ''),
						Number(args.limit || 10)
					)

				case 'get_api_details':
					return await this.searchService.getAPIDetails(
						String(args.apiName || '')
					)
				...
				case 'get_random_api':
					return await this.searchService.getRandomAPI(
				...
				case 'find_alternative_apis':
					return await this.recommendationService.findAlternativeAPIs(
						String(args.functionality || ''),
						Number(args.limit || 5)
					)

				case 'generate_api_integration_code':
					return await this.codeGenerationService.generateAPIIntegrationCode(
						String(args.apiName || ''),
						String(args.language || 'javascript')
					)

				case 'sync_repository_data':
					const syncResult = await this.apiService.syncRepositoryData(
				...
				case 'check_new_apis':
					return await this.recommendationService.checkNewAPIs(
						Number(args.days || 7)
					)
