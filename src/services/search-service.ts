...
export class SearchService {
   ...
   * 按分类搜索API
   */
  public async searchAPIsByCategory(category: string, limit: number): Promise<ToolResponse> {
    ...
    if (!categoryData) {
      ...
      if (matchedCategory) {
        ...
        return {
          content: [{
            type: 'text',
            text: `找到分类 "${matchedCategory}"，共 ${apiDatabase[matchedCategory].count} 个API：\n\n` + 
                  results.map(api => this.formatterService.formatAPIEntry(api)).join('\n\n')
          }]
        };
      }
    ...
    return {
      content: [{
        type: 'text',
        text: `${category} 分类中的API（显示前${limit}个，共${categoryData.count}个）：\n\n` + 
              results.map(api => this.formatterService.formatAPIEntry(api)).join('\n\n')
      }]
    };
  }
   ...
   * 按关键词搜索API
   */
  public async searchAPIsByKeyword(keyword: string, limit: number): Promise<ToolResponse> {
    const apiDatabase = this.apiService.getApiDatabase();
    ...
    for (const categoryData of Object.values(apiDatabase)) {
      for (const api of categoryData.entries) {
        if (api.API.toLowerCase().includes(searchTerm) || 
    ...
    return {
      content: [{
        type: 'text',
        text: `包含关键词 "${keyword}" 的API（共找到${results.length}个）：\n\n` + 
              results.map(api => this.formatterService.formatAPIEntry(api)).join('\n\n')
      }]
    };
  }
   ...
   * 按认证类型筛选API
   */
  public async filterAPIsByAuth(authType: string, limit: number): Promise<ToolResponse> {
    const apiDatabase = this.apiService.getApiDatabase();
    const results: APIEntry[] = [];
    ...
    return {
      content: [{
        type: 'text',
        text: `认证类型为 "${authType}" 的API（共找到${results.length}个）：\n\n` + 
              results.map(api => this.formatterService.formatAPIEntry(api)).join('\n\n')
      }]
    };
  }
   ...
   * 按HTTPS支持筛选API
   */
  public async filterAPIsByHTTPS(httpsOnly: boolean, limit: number): Promise<ToolResponse> {
    const apiDatabase = this.apiService.getApiDatabase();
    const results: APIEntry[] = [];
    ...
    return {
      content: [{
        type: 'text',
        text: `${httpsStatus} HTTPS 的API（显示前${limit}个）：\n\n` + 
              results.map(api => this.formatterService.formatAPIEntry(api)).join('\n\n')
      }]
    };
  }
   ...
   * 按CORS支持筛选API
   */
  public async filterAPIsByCORS(corsSupport: string, limit: number): Promise<ToolResponse> {
    const apiDatabase = this.apiService.getApiDatabase();
    const results: APIEntry[] = [];
    ...
    return {
      content: [{
        type: 'text',
        text: `CORS支持状态为 "${corsSupport}" 的API（显示前${limit}个）：\n\n` + 
              results.map(api => this.formatterService.formatAPIEntry(api)).join('\n\n')
      }]
    };
  }
  ...
  public async getCategoryList(): Promise<ToolResponse> {
    ...
    const categoryInfo = categories.map(category => 
      `${category} (${apiDatabase[category].count}个API)`
    );
    
    return {
      content: [{
        type: 'text',
        text: `可用的API分类（共${categories.length}个）：\n\n${categoryInfo.join('\n')}`
      }]
    };
  }
  ...
  public async getRandomAPI(category?: string): Promise<ToolResponse> {
    ...
    return {
      content: [{
        type: 'text',
        text: `随机推荐的API：\n\n${this.formatterService.formatAPIDetails(randomAPI)}`
      }]
    };
  }
  ...
  public async getAPIStatistics(): Promise<ToolResponse> {
    ...
    const totalAPIs = Object.values(apiDatabase).reduce((sum, data) => sum + data.count, 0);
    ...
    const authInfo = Object.entries(authTypes)
      .sort((a, b) => b[1] - a[1])
      .map(([type, count]) => `${type}: ${count}个 (${((count / totalAPIs) * 100).toFixed(1)}%)`)
      .join('\n');
    ...
    return {
      content: [{
        ...
        text: `API统计信息：\n\n` +
              ...
              `总API数：${totalAPIs}\n\n` +
              `认证类型分布：\n${authInfo}\n\n` +
              `HTTPS支持：${httpsCount}个 (${((httpsCount / totalAPIs) * 100).toFixed(1)}%)\n\n` +
              `CORS支持分布：\n${corsInfo}`
      }]
    };
  }
  ...
  public async analyzeAuthRequirements(): Promise<ToolResponse> {
    ...
    for (const categoryData of Object.values(apiDatabase)) {
      for (const api of categoryData.entries) {
        ...
        if (authAnalysis[api.Auth].apis.length < 5) { // 每种认证类型最多列出5个示例
    ...
    for (const [authType, data] of Object.entries(authAnalysis).sort((a, b) => b[1].count - a[1].count)) {
